...
class CLI {
  constructor(options) {
    ...
    this.cacheDir = '';
    this.packageJSON = '';
    this.lockFile = new LockFile(this.cwd);
  ...
  init() {
    ...
    this.lockFile = new LockFile(this.cacheDir);
  ...
  async start() {
    if (!await this.lockFile.open()) {
        throw new Error('Another process is running.\n'
          + `If not remove: ${this.lockFile.filename}.`);
    }
