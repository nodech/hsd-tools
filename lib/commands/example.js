...
class ExampleCommand extends Command {
  ...
  async verifyDeps() {
    ...
    for (const dep of deps) {
      promises.push(sem.do(async () => {
        this.step(S_VERIFY(dep), RUNNING, 'Checking npm..');
        await timeout();
  ...
  async run() {
    this.task(T_CHECK_REMOTES, null, 'Waiting..');
    this.task(T_VERIFY_DEPS, null, 'Waiting..');
    this.task(T_CHECK_REMOTES_2, null, 'Waiting..');
