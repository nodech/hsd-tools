...
class GitPRLog extends Command {
  ...
  async gatherGithubData() {
    ...
    for (const pr of allRemotePRs) {
      steps.push([STEP_GETTING_PR(pr), async () => {
        ...
        try {
          const res = await this.githubAPI.getPRInfo(owner, repo, pr);
          info = res[0];
          cached = res[1];
        } catch (e) {
          throw new this.Error(e.message);
        }
  ...
  isMerge(hash) {
    ...
    //   this.log(`!!! WTF IS WRONG with ${hash}, `
    //    + `merge msg: ${msgMerge}, pr: ${prs}`);
  ...
  outputGroupedByPR() {
    ...
    const formatMsg = (hash, message, pr = false) => {
      let fhash = hash;

      if (!this.longID)
        fhash = fhash.substr(0, 8);

      const prno = this.ghdataPRByHash.get(hash);

      if (!pr || !prno)
        return [`${fhash} - ${message}`];

      const info = this.ghdata.get(prno);
      return [
        `${fhash} - ${message} - ${info.title} - @${info.user.login}`
      ];
    };
