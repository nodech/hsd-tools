...
const HTOOLS_SEED = 'seed.htools.work';
const BCOIN_NINJA = 'hs-mainnet.bcoin.ninja';
...
class GenerateSeeds extends Command {
  constructor(options) {
    ...
    this.filterHeight = options.filterHeight ?? 0;

    /** @type {Set<string>} */
    this.bcoinNinja = new Set();

    /** @type {Set<string>} */
    this.htoolsWork = new Set();
  ...
  async getHtoolsWorkSeeds() {
    const seeds = await resolve(HTOOLS_SEED);

    for (const seed of seeds) {
      const addr = `${seed}:${PORT_CLEAR}`;
      this.htoolsWork.add(addr);
      this.allSeeds.add(addr);
    }
  }

  async getBcoinNinjaSeeds() {
    const seeds = await resolve(BCOIN_NINJA);

    for (const seed of seeds) {
      const addr = `${seed}:${PORT_CLEAR}`;
      this.bcoinNinja.add(addr);
      this.allSeeds.add(addr);
    }
  }
  ...
  seedDataFromEntry(entry) {
    ...
    const seedData = {
      ...
      htoolsDNS: this.htoolsWork.has(addr),
      bcoinNinja: this.bcoinNinja.has(addr),
  ...
  async getSeeds() {
    ...
    const listSteps = {
      'bcoin.ninja': () => this.getBcoinNinjaSeeds(),
      'htools.work': () => this.getHtoolsWorkSeeds(),
  ...
  formatSeedData(entry) {
    ...
    tableEntry.htoolsDNS = formatYN(entry.htoolsDNS);
    tableEntry.ninja = formatYN(entry.bcoinNinja);
  ...
  filter(data) {
    ...
    const filterHeight = (d) => {
      return d.entry.lastHeight >= this.filterHeight;
    };
  ...
  outputSeeds() {
    ...
    const newgens = data.filter((d) => {
      return !d.entry.isMainSeed;
    });

    this.log('module.exports = [');
    this.log(' ...');

    for (const entry of newgens) {
      this.log(` '${entry.seed.split(':')[0]}',`);
    }
